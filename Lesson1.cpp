...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
  ...
  Point tf00 = f(Point(0, 0));
  Point tf10 = f(Point(1, 0));
  Point tf01 = f(Point(0, 1));
  Point tf11 = f(Point(1, 1));


  glBegin(GL_LINE_STRIP);
  glVertex2d(tf00.x, tf00.y);
  glVertex2d(tf01.x, tf01.y);
  glVertex2d(tf11.x, tf11.y);
      ...
      if (GetTouchInputInfo((HTOUCHINPUT)lParam, cInputs, pInputs, sizeof(TOUCHINPUT))){
        ...
        for (int i = 0; i < static_cast<INT>(cInputs); i++, j++) {
          ...
          if (ti.dwID != 0){
            ...
            Point p = Point(ptInput.x, client.bottom - ptInput.y);
            if (j == 0)
              t.p1 = PointOrNull(p.x, p.y);
            else
              t.p2 = PointOrNull(p.x, p.y);
          }
        } // for touchpoints
